...
class Date
  ...
  class << self
    ...
    # <tt>config.beginning_of_week</tt>.
    ...
    def find_beginning_of_week!(week_start)
      return week_start if ::Date::DAYS_INTO_WEEK.key?(week_start)

      raise ArgumentError, "Invalid beginning of week: #{week_start}"
    end
  ...
  # to the beginning of the day (0:00) and then subtracts the specified number
  # of seconds.
  def ago(seconds)
  ...
  # +options+ parameter takes a hash with any of these keys: <tt>:years</tt>,
  # <tt>:months</tt>, <tt>:weeks</tt>, <tt>:days</tt>.
  def advance(options)
    options = options.dup
    d = self
  ...
  # The +options+ parameter is a hash with a combination of these keys:
  # <tt>:year</tt>, <tt>:month</tt>, <tt>:day</tt>.
  #
